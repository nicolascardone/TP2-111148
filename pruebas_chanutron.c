...
void crear_juego() {
  ...
  pa2m_afirmar(juego_finalizado(j) == false, "El juego no está finalizado");
  juego_destruir(j);
}
...
void cargar_archivo() {
  ...
  pa2m_afirmar(juego_cargar_pokemon(j, ARCHIVO_INEXISTENTE) == ERROR_GENERAL,
               "Cargar un archivo inexistente resulta en ERROR_GENERAL");
  pa2m_afirmar(lista_tamanio(juego_listar_pokemon(j)) == 0,
               "El listado de pokemones es vacío");
  pa2m_afirmar(juego_cargar_pokemon(j, ARCHIVO_CORTO) == POKEMON_INSUFICIENTES,
...
void seleccionar() {
  ...
  pa2m_afirmar(lista_tamanio(juego_listar_pokemon(j)) == 6,
               "El listado de pokemones tiene el tamaño correcto");
  pa2m_afirmar(juego_seleccionar_pokemon(j, JUGADOR1, "Pikachu", "Charmander",
  ...
  pa2m_afirmar(juego_seleccionar_pokemon(j, JUGADOR1, "Pikachu", "sarasa",
                                         "Charmander") == POKEMON_INEXISTENTE,
  ...
  pa2m_afirmar(juego_seleccionar_pokemon(j, JUGADOR2, "Charmander", "Pikachu",
                                         "Cacnea") == TODO_OK,
...
void jugar() {
  ...
  pa2m_afirmar(juego_seleccionar_pokemon(j, JUGADOR2, "Floatzel", "Togepi",
                                         "Larvitar") == TODO_OK,
  ...
  pa2m_afirmar(
      (obtenido = juego_jugar_turno(j, validaj12, validaj22)).jugador1 ==
          ATAQUE_EFECTIVO,
      "Realizar una jugada valida de tipo ELECTRICO contra un pokemon AGUA "
      "resulta en ataque efectivo");
  pa2m_afirmar(obtenido.jugador2 == ATAQUE_INEFECTIVO,
  ...
  pa2m_afirmar(
      (obtenido = juego_jugar_turno(j, validaj13, validaj23)).jugador1 ==
          ATAQUE_EFECTIVO,
