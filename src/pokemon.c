...
informacion_pokemon_t *pokemon_cargar_archivo(const char *path) {
  ...
  while (fgets(linea, 500, archivo) != NULL) {
    if (!poke) {
      if (sscanf(linea, "%[^;];%c", pokemon.nombre, &tipo) == 2) {
        switch (tipo) {
        case 'N':
          pokemon.tipo = NORMAL;
          break;
      ...
      if (sscanf(linea, "%[^;];%c;%u", ataque.nombre, &tipo, &ataque.poder) ==
          3) {
        switch (tipo) {
        case 'N':
          ataque.tipo = NORMAL;
    ...
    if (contador_ataques == 3) {
      ...
      info->pokemones = realloc(info->pokemones,
                                (size_t)(info->cantidad) * sizeof(pokemon_t));
...
int con_cada_pokemon(informacion_pokemon_t *ip, void (*f)(pokemon_t *, void *),
  ...
  for (int n = 0; n < ip->cantidad; n++) {
    for (int p = 0; p < ip->cantidad - n - 1; p++) {
      if (comparar(pokemones[p].nombre, pokemones[p + 1].nombre) > 0) {
