...
JUEGO_ESTADO juego_cargar_pokemon(juego_t *juego, char *archivo) {
  ...
  if (juego->jugador1->lista == NULL || juego->jugador2->lista == NULL ||
      juego->jugador1->ataques == NULL || juego->jugador2->ataques == NULL) {
    return ERROR_GENERAL;
  }
  return TODO_OK;
}
...
resultado_jugada_t juego_jugar_turno(juego_t *juego, jugada_t jugada_jugador1,
  ...
  pokemon_t *pokemon1 =
      lista_buscar_elemento(juego->jugador1->lista, comparador, (void *)poke);
  pokemon_t *poke2 = pokemon_buscar(juego->info, jugada_jugador2.pokemon);
  pokemon_t *pokemon2 =
      lista_buscar_elemento(juego->jugador2->lista, comparador, (void *)poke2);
  ...
  if (!error1) {
    ...
    if (error1 == false) {
      resultado.jugador1 =
          calcular_resultado_jugada(pokemon2, ataque1, &resultado1);
      juego->jugador1->puntaje += (int)resultado1;
      resultado.jugador2 =
          calcular_resultado_jugada(pokemon1, ataque2, &resultado2);
      juego->jugador2->puntaje += (int)resultado2;
      juego->rondas++;
      for (size_t i = 0; i < lista_tamanio(juego->jugador1->ataques); i++) {
        if (lista_elemento_en_posicion(juego->jugador1->ataques, i) ==
            (void *)ataque1) {
      ...
      for (size_t n = 0; n < lista_tamanio(juego->jugador1->ataques); n++) {
        if (lista_elemento_en_posicion(juego->jugador2->ataques, n) ==
            (void *)ataque2) {
