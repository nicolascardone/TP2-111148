				       ...
				       const char *nombre3)
{
	...
	if (jugador == JUGADOR1) {
		juego->jugador1->lista =
			lista_insertar(juego->jugador1->lista, (void *)poke1);
		juego->jugador1->lista =
			lista_insertar(juego->jugador1->lista, (void *)poke2);
		juego->jugador2->lista =
			lista_insertar(juego->jugador2->lista, (void *)poke3);
		...
		juego->jugador2->lista =
			lista_insertar(juego->jugador2->lista, (void *)poke1);
		juego->jugador2->lista =
			lista_insertar(juego->jugador2->lista, (void *)poke2);
		juego->jugador1->lista =
			lista_insertar(juego->jugador1->lista, (void *)poke3);
...
RESULTADO_ATAQUE calcular_resultado_jugada(pokemon_t *pokemon,
...
{
	void *elemento =
		lista_buscar_elemento(ataques, comparador, (void *)ataque);
	if (elemento != NULL) {
		return false;
	}
	return true;
				     ...
				     jugada_t jugada_jugador2)
{
	...
	pokemon_t *pokemon1 = lista_buscar_elemento(juego->jugador1->lista,
						    comparador, (void *)poke);
	pokemon_t *poke2 = pokemon_buscar(juego->info, jugada_jugador2.pokemon);
	pokemon_t *pokemon2 = lista_buscar_elemento(juego->jugador2->lista,
						    comparador, (void *)poke2);
	...
	if (!error1) {
		...
		if (error1 == false) {
			resultado.jugador1 = calcular_resultado_jugada(
				pokemon2, ataque1, &resultado1);
			juego->jugador1->puntaje += (int)resultado1;
			resultado.jugador2 = calcular_resultado_jugada(
				pokemon1, ataque2, &resultado2);
			juego->jugador2->puntaje += (int)resultado2;
			juego->rondas++;
			for (size_t i = 0;
			     i < lista_tamanio(juego->jugador1->ataques); i++) {
				if (lista_elemento_en_posicion(
					    juego->jugador1->ataques, i) ==
				    (void *)ataque1) {
			...
			for (size_t n = 0;
			     n < lista_tamanio(juego->jugador1->ataques); n++) {
				if (lista_elemento_en_posicion(
					    juego->jugador2->ataques, n) ==
				    (void *)ataque2) {
					lista_quitar_de_posicion(
						juego->jugador2->ataques, n);
					break;
