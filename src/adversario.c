				    ...
				    char **nombre2, char **nombre3)
{
	...
	pokemon_t *poke1 = lista_elemento_en_posicion(lista_aux,(size_t)posicion);
	...
	pokemon_t *poke2 = lista_elemento_en_posicion(lista_aux,(size_t)posicion2);
...
{	
	...
	/*for(size_t i = 0; i < 9; i++){
		if(i < 3){
			pokemon_t *poke = lista_elemento_en_posicion(adversario->pokemones_adversario,i);
			printf("%s \n",pokemon_nombre(poke));;
		}
		struct ataque *ataque = lista_elemento_en_posicion(adversario->ataques,i);
		printf(" \t%s \n",ataque->nombre);
	}*/
	lista_t *lista = lista_crear();
	poke = lista_elemento_en_posicion(adversario->pokemones_adversario,(size_t)adversario->rondas);		
