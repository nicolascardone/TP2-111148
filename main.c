...
int main(int argc, char *argv[]) {
  ...
  pedir_pokemones_jugador(&eleccionJugador1, &eleccionJugador2,
                          &eleccionJugador3);
  if (eleccionJugador1 == NULL || eleccionJugador2 == NULL ||
      eleccionJugador3 == NULL) {
    juego_destruir(juego);
    free(archivo);
  ...
  JUEGO_ESTADO estado_seleccion = juego_seleccionar_pokemon(
      juego, JUGADOR1, eleccionJugador1, eleccionJugador2, eleccionJugador3);
  ...
  while (!juego_finalizado(juego)) {
    ...
    jugada_adversario = adversario_proxima_jugada(adversario);
    resultado_ronda =
        juego_jugar_turno(juego, jugada_jugador, jugada_adversario);
